...
public class JFlapEquivalenceTester {
    ...
    public void testEquivalent() {
        ...
        if (!expectedMinusActual.isEmpty()) {
            fail(String.format("ei aktsepteeri '%s', aga peaks", expectedMinusActual.getShortestExample(true)));
        }

        Automaton actualMinusExpected = actualAutomaton.minus(expectedAutomaton);
        if (!actualMinusExpected.isEmpty()) {
            fail(String.format("aktsepteerib '%s', aga ei tohiks", actualMinusExpected.getShortestExample(true)));
        }
