...
public class OttEvaluatorTest {
    ...
    public void test07_assign_value() {
        checkEval(seq(assign("x", assign("y", num(42))), var("x")), trueOracle, 42);
        checkEval(seq(assign("x", assign("y", num(42))), var("y")), trueOracle, 42);
        checkEval(add(assign("x", num(10)), num(5)), trueOracle, 15);

        checkEval(assign("foo", num(42)), trueOracle, 42);
        checkEval(assign("foo", decision(num(42), num(24))), altOracle(true), 42);
    }

    @Test
    public void test08_multiple() {
        checkEval(seq(assign("foo", num(42)), var("foo")), trueOracle, 42);
        checkEval(seq(assign("foo", seq(assign("x", add(var("x"), num(1))))), var("x")), trueOracle, 1);
        ...
        checkEval(expr, trueOracle, 13 + 10);
        checkEval(expr, falseOracle, 11 + 12);
        checkEval(expr, altOracle(true), 13 + 12);
        checkEval(expr, altOracle(false), 11 + 10);
    }
